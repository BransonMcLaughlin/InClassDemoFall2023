public class FriendsList extends ListADT<Friend>{
    ...
    public void add(int index, Friend item) throws ListException {
        ...
        if(index == 0){//start of list
            newFriend.setNext(this.head);// makes the current index 0 the next address 
            this.head = newFriend;// becomes the head of the list
        } else {
    ...
    public void remove(int index) throws ListException {
        ...
        if(index == 0) {// remove the first element
            ...
            Node<Friend> previous = this.getNodeAt(index - 1);
            Node<Friend> nodeToRemove = previous.getNext();// change where the nodes are pointing to their next node
